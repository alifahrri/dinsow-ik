...
void SceneModifier::setTarget(double x, double y, double z)
{
  if(!dinsow)
    return;
  std::cout << "[SceneModifier]->setTarget" << std::endl;
//  target->transform->setTranslation(QVector3D(x,y,z));
  QVector<double> target_frame;
  target_frame.resize(12);
  if(y > 0)
    {
      target_frame[0] = x;
      target_frame[1] = y;
      target_frame[2] = z;
      target_frame[3] = -3.14/2;
      target_frame[6] = 25;
      target_frame[7] = -15;
      target_frame[8] = -25;
    }
  ...
  for(size_t i=0; i<6; i++)
    j.q[i] = 0.0;
  dinsow->setJoints(DinsowKinematic::LEFT, j);
  dinsow->setJoints(DinsowKinematic::RIGHT, j);

  this->dinsowIK(target_frame);
}
...
{
  target = new JointEntity(rootEntity, 0.99);
  link.resize(14);
  joint.resize(14);
